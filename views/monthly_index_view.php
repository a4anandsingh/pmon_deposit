...
<div id="content_wrapper">
    ...
    <div style="width:100%;float:left">
	     <?php  if($isValid) $message = '<big><big style="color:#f00">अभी आपकी कुल '.$isValid.' परियोजना (प्रोमोन या रिमॉडलिंग में) Setup या Financial and Physical Target Setup स्तर पर ही है इसलिए आप मासिक प्रविष्टी नहीं कर पा रहे हैं।</big></big>'; 
			echo getMessageBox('message', $message); ?>
    </div>
	<div style="width:100%;float:left;margin-bottom:5px" align="center">
    	<a target="_blank" href="<?php echo base_url();?>documents/help/promon__deposit_monthly.pdf" class="btn btn-primary">
...
function showMonthlyStatusForm(project_id, dtval){
	var params = {
		...
		'data':{'PROJECT_ID':project_id, 'date_val':dtval}, 
...
function showMonthlyStatusFormMi(project_id, dtval){
	var params = {
		'divid':'divMonthlyDataEntry',
		'url':'showMonthlyStatusFormMi',
		'data':{'PROJECT_ID':project_id, 'date_val':dtval},
		'donefname': 'doneMonthly',
		'failfname' :'failMonthly',
		'alwaysfname':'none'
...
function lockMonthlyMi(project_id, dt){
	window.myProjectId = project_id;
	window.myDate = dt;
	var res = confirm("अनलॉक के उपरान्त लॉक करना चाहते है \nतो कृपया मासिक प्रविष्टि को कम से कम एक बार सेव करना सुनिश्चित करें \n\n" +
		" अगर आपने मासिक प्रविष्टि सेव नहीं किया है तो Cancel बटन दबाएं \nमासिक प्रविष्टि लॉक करने के लिए OK बटन दबाएं");
	if(res){
		var params = {
			'divid':'mySaveDiv',
			'url':'monthlyProgressCheckMi',
			'data':{'project_id':project_id, 'lock_month':dt},
			'donefname': 'doneLockCheckingMi',
			'failfname' :'',
			'alwaysfname':'none'
		};
		callMyAjax(params);
...
function doneLockCheckingMi(response){
	if(response>110){
		alert('परियोजना में भौतिक प्रगति 110 प्रतिशत से ज्यादा है अतः इस माह के डाटा को लॉक नहीं किया जा सकता');
	}else{
		goForLockMi();
...
function goForLockMi(project_id, month){
	var params = {
		'divid':'mySaveDiv',
		'url':'lockMonthlyMi',
		'data':{'project_id':window.myProjectId, 'lock_month':window.myDate},
