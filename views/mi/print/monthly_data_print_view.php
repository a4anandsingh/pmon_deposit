...
$arrMonthly = array(
	...
	array(
		'SNO'=>7,
		'LIST_VALUE'=>'7',
		'TITLE'=>'Earthwork (As per "L" Earthwork section of DPR)',
		'UNIT'=>'Th Cum',
	...
	array(
		...
		'SHOW'=> $setupData['C_WATERPUMP_NA']
	),
	array(
		'SNO'=>13,
		'LIST_VALUE'=>'13',
		'TITLE'=>'Building Works<br>(As per "K" Building sectin of DPR) Control Rooms',
		'UNIT'=>'Numbers',
...
foreach($arrMonthly as $arrM){
	...
	if($arrM['SNO']==$iSNOofIPRow) {
		...
		if($arrM['SHOW']==1){
			$contentOne .= str_repeat('<td align="center" class="ui-widget-content">NA</td>', 7).'</tr>';
		}else{
			$contentOne .= '<td align="center" class="ui-widget-content" colspan="7"></td></tr>';
			$blockData = '';
			$iBCount = 97;
			foreach($arrBlockData as $k=>$v){
				...
				$blockData .= '<tr>
					...
					<td class="ui-widget-content" rowspan="3" colspan="3">'.$v['BLOCK_NAME'].'</td>
			...
			<tr>
				...
				<td align="center" class="ui-state-default">'.($arrM['KHARIF']['E_VALUE']+$arrM['RABI']['E_VALUE']).'</td>
    ...
    if($arrM['SNO']>=8 && $arrM['SNO'] <=12){
        if($arrM['SNO']==8) {
            $contentOne .= '<td class="ui-widget-content" rowspan="5" align="center"><strong>'.$sno.'</strong></td>';
            $contentOne .= '<td class="ui-widget-content"  rowspan="5"><strong>Masonry/Concrete <br />(As per "C" Masonry Section of DPR) </strong></td>';
        }
    ...
    $contentOne .= '<td class="ui-widget-content">'.$arrM['UNIT'].'</td>';
    $contentOne .= '<td class="ui-widget-content"></td>';

	/*$rowSpan = '';
	if($arrM['SNO']==$iSNOofIPRow) $rowSpan = 'rowspan="3"';
	$contentOne .= '<tr>
		<td class="ui-widget-content" '.$rowSpan.' align="center"><strong>'.$sno.'</strong></td>
		<td colspan="2" class="ui-widget-content" '.$rowSpan.'><strong>'.$arrM['TITLE'].'</strong></td>
		<td class="ui-widget-content" '.$rowSpan.'>'.$arrM['UNIT'].'</td>';
		
	if(($arrM['SHOW']==1) || (substr($arrM['TITLE'], 0, 5)=='Expen')){//
		$contentOne .= str_repeat('<td align="center" class="ui-widget-content">NA</td>', 6).'</tr>';
		continue;
	}
	*/
...
<table width="100%" border="0" cellpadding="3" cellspacing="2" class="ui-widget-content">
<tr>
    <td class="ui-widget-header" colspan="4"><strong>2] Status of Monthly</strong></td>
</tr>
<tr>
