<title>Financial and Physical Target Setup - Micro Irrigation - WRD MIS</title>
...
<div id="content_wrapper">
	...
	<div style="width:100%;float:left;margin-bottom:5px" align="center">
    	<a target="_blank" href="<?php echo base_url();?>documents/help/promon_financial_and_physical_target_setup.pdf" 
...
function gridReload(mode){
	...
	//$.extend(true, pdata, xdata);
	$("#projectListGrid").jqGrid('setGridParam',gparam).trigger("reloadGrid"); 
	//jQuery("#projectListGrid").setGridParam({postData:pdata1, page:1}).trigger('reloadGrid');
}
//
...
function doneTarget(response){
	...
	$('#BUDGET_AMOUNT').focus();
	$('html, body').animate({scrollTop: $("#divTargetForm").offset().top}, 1000);
