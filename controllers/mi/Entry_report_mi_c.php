...
class Entry_report_mi_c extends Project_library {
    ...
    function __construct(){
		...
		$this->load->model('pmon_dep/dep_mi__m_report','mi__m_report');
	}
	...
	public function getTargetSessionOptions(){
		...
		$recs = $this->db->distinct()
			...
			->from('dep_mi__t_yearlytargets as target')
	...
	public function printSetup(){
        ...
        array_push($projectSetupFields,'PROJECT_SUB_TYPE' );
        array_push($projectSetupFields,'PROJECT_TYPE' );

        array_push($projectSetupFields,'HEAD');
        array_push($projectSetupFields,'SCHEME_NAME_HINDI');
        array_push($projectSetupFields,'SCHEME_NAME_ENGLISH');
		...
		//get sdo list
		$sdo = array();
		$recs = $this->db->get_where('dep_mi__v_project_sdo', $arrWhere);
		if($recs && $recs->num_rows()){
			$xi = 1;
		...
		if($this->PROJECT_SETUP_ID) {
			...
			*/
			...
			//showArrayValues($recEstimation); exit;
		...
		if($this->PROJECT_SETUP_ID > 0) {
			$recs = $this->db->get_where('dep_mi__m_projects_office', $arrWhere);
			if($recs && $recs->num_rows()) {
				foreach ($recs->result() as $rec) {
					$EE_ID = $rec->EE_ID;
		...
		if($this->PROJECT_SETUP_ID) {
			...
			$data['ASSEMBLY_BENEFITED'] = $this->mi__m_report->getAssemblys($this->getBenefitedAssemblyIDsMi($this->PROJECT_SETUP_ID));
		...
		//for tubewell id is 5
		if($projectSetupValues['PROJECT_SUB_TYPE_ID']==5){
            $myview = $this->load->view('pmon_deposit/mi/print/project_setup_data_tubewell_print_view_table', $data, true);
        }else{
            $myview = $this->load->view('pmon_deposit/mi/print/project_setup_data_print_view_table', $data, true);
        }
		array_push($this->message, getMyArray(null, $myview));
        echo $xno.'####'.createJSONResponse($this->message);
	}
    protected function getBenefitedAssemblyIDsMi($projectId){
        $ids = array();
        if($projectId){
            $recs = $this->db->get_where('dep_mi__m_assembly_const_served', array('PROJECT_SETUP_ID'=>$projectId));
    ...
    public function printTarget(){
		...
		$miProjectData = $this->mi__m_report->getMiProjectData(
		    array('PROJECT_NAME', 'PROJECT_CODE', 'AA_AMOUNT', 'AA_DATE', 'PROJECT_COMPLETION_DATE','PROJECT_SUB_TYPE_ID'),$arrWhere);
		...
		$data['PROJECT_NAME'] = $miProjectData['PROJECT_NAME'];
		...
		//$myview = $this->load->view('pmon_deposit/mi/print/target_data_print_view', $data, true);
        //showArrayValues($miProjectData); exit;
        if($miProjectData['PROJECT_SUB_TYPE_ID']==5){
            //$myview = $this->load->view('pmon_deposit/mi/print/project_setup_data_tubewell_print_view_table', $data, true);
            $myview = $this->load->view('pmon_deposit/mi/print/target_data_tubewell_print_view', $data, true);
        }else{
            //$myview = $this->load->view('pmon_deposit/mi/print/project_setup_data_print_view_table', $data, true);
            $myview = $this->load->view('pmon_deposit/mi/print/target_data_print_view', $data, true);
        }
    ...
    public function printMonthly(){
        ...
        $monthlyTable = 'dep_mi__t_monthly';
		...
		//echo 'entry date= '. $entryDate; exit;
        ...
        $recs = $this->db->get_where('dep_mi__t_monthlydata_remarks', $arrWhich);
        ...
        $recs = $this->db->get_where('dep_mi__t_progress', array(
			'PROJECT_SETUP_ID' => $this->PROJECT_SETUP_ID,
        ...
        $recs = $this->db->get_where('dep_mi__t_status_date', $arrWhich);
        if($recs && $recs->num_rows()) {
            $rec = $recs->row();
			$recs->free_result();
            foreach($mMonthlyStatusFields as $f)
		...
		$recs = $this->db->get_where('dep_mi__m_setup_status', $arrWhere);
		if($recs && $recs->num_rows()) {
			$rec = $recs->row();
			$recs->free_result();
			foreach($mAFStatus as $f){
		...
		if($currentFinancialMonth != 0) {
			...
			$strSQL = 'SELECT '.$sumField.' FROM dep_mi__t_monthly WHERE PROJECT_SETUP_ID=' . $this->PROJECT_SETUP_ID .
        ...
        $strSQL = 'SELECT e.*, p.RAA_DATE FROM dep_mi__t_estimated_qty as e 
				LEFT JOIN dep_mi__t_raa_project as p on(p.RAA_PROJECT_ID=e.RAA_ID)
        ...
        if($recs && $recs->num_rows()) {
            ...
            $this->db->select( implode(',',$arrFieldsForProgress).',KHARIF,RABI, SUM(KHARIF+RABI) as IP_TOTAL')
                    ->from('dep_mi__t_estimated_qty')
                    ->join('dep_mi__ip_design','dep_mi__ip_design.PROJECT_SETUP_ID=dep_mi__t_estimated_qty.PROJECT_SETUP_ID','left')
                    ->where_in('dep_mi__t_estimated_qty.PROJECT_SETUP_ID', $this->PROJECT_SETUP_ID)
                    ->where('ADDED_BY', 0);
            $recs = $this->db->get();
            //echo  $this->db->last_query(); exit;
        ...
        $data['SUBMISSION_DATE'] = '';
        $recs = $this->db->get_where('dep_mi__t_yearlytargets', $arrWhich);
        //echo $this->db->last_query();
        //showArrayValues($rec);
        if($recs && $recs->num_rows()) {
            $rec = $recs->row();
        ...
        $arrWhere = array('PROJECT_SETUP_ID' => $this->PROJECT_SETUP_ID);
        $this->db->select('PROJECT_COMPLETION_DATE, AA_DATE AS PROJECT_START_DATE, PROJECT_NAME, PROJECT_NAME_HINDI,PROJECT_CODE');
        $recs = $this->db->get_where('dep_mi__m_project_setup', $arrWhere);
        if($recs && $recs->num_rows()) {
            $rec = $recs->row();
            $data['ACTUAL_COMPLETION_DATE'] = $rec->PROJECT_COMPLETION_DATE;
            $data['PROJECT_START_DATE'] = $rec->PROJECT_START_DATE;
            $data['PROJECT_NAME'] = $rec->PROJECT_NAME . ' - ' . $rec->PROJECT_NAME_HINDI;
        ...
        */
        ...
        //echo 'aa';
        $arrProjectType = $this->mi__m_report->getProjectSubTypeId($this->PROJECT_SETUP_ID);
        //showArrayValues($arrProjectType ); exit;
        // 5- tubewell, 25- microirrigation
        if($arrProjectType['PROJECT_SUB_TYPE_ID']==5){
            $myview = $this->load->view('mi/print/tubewell_monthly_data_print_view_table', $data, true);
        }else{
            $myview = $this->load->view('mi/print/mi_monthly_data_print_view_table', $data, true);
        }
        //$myview = $this->load->view('mi/print/monthly_data_print_view', $data, true);
        //$myview = $this->load->view('mi/print/monthly_data_print_view_table', $data, true); //commented on 03-06-2019
        //echo $myview ;
        array_push($this->message, getMyArray(null, $myview));
    ...
    public function printMP()
    {
        ...
        $recs = $this->db->get_where('dep_mi__t_progress', array('PROJECT_SETUP_ID' => $projectId));
