...
class Project_c extends Project_library{
		...
		if ($EE_ID==0 && $SE_ID==0 && $CE_ID==0){
			...
			$objFilter->SQL = 'SELECT PROJECT_SETUP_ID, PROJECT_ID, PROJECT_START_DATE,
				...
				IF(SETUP_LOCK=1, "<span class=\'cus-lock\'></span>",
					"<span class=\'cus-bullet-green\'></span>") as LOCKED, CONCAT(HEAD," ",SCHEME_NAME_HINDI) AS HEAD
	...
	public function getProjectListGrid1(){
		...
		if ($EE_ID==0 && $SE_ID==0 && $CE_ID==0){
			...
			$objFilter->SQL = 'SELECT 
					...
					"<span class=\'cus-bullet-green\'></span>") as LOCKED, HEAD
	...
	private function showProjectSetupData(){
		...
		if($ProjectType==1){
            ...
            if($this->session->userdata('HOLDING_PERSON')==4){
                ...
                $SDO_IDs = array();
                if($editMode){
                    $recsSdoEE = $this->db->get_where('deposit__projects_office', $arrWhere);
	...
	public function getBlockBenefitedList(){
		...
		$arecs = $this->db->get_where(
			'deposit__projects_block_served', 
	...
	//showArrayValues($data);exit;
		if($editMode){
			@$this->db->update('deposit__projects', $data, $arrWhich);
		}else{
			$data['ENTRY_FROM'] = 6;
			@$this->db->insert('deposit__projects', $data);
		...
		if ($this->db->affected_rows()){
			...
			if($editMode){
				...
				$strSQL = "INSERT INTO deposit__t_locks (PROJECT_ID, SETUP_LOCK) 
	...
	private function saveIrrigationPotential(){
		...
		if($recs && $recs->num_rows()){
			@$this->db->update('deposit__agriculture', $dataa, $arrWhich);	
		}else{
          	$data['PROJECT_ID'] = $this->PROJECT_ID;
          	@$this->db->insert('deposit__agriculture', $dataa);	
        }
		//save general data
		...
		if($recs && $recs->num_rows()){
			@$this->db->update('deposit__general_data', $data, $arrWhich);
		}else{
          	$data['PROJECT_ID'] = $this->PROJECT_ID;
          	@$this->db->insert('deposit__general_data', $data);
        }
	}
	...
	public function deleteProject(){
		...
		$projectTables = array(
			'deposit__projects_office', 
			'deposit__projects_assembly_const_served', 
			'deposit__projects_block_served', 
			'deposit__projects_district_served', 
			'deposit__projects_villages_served', 
			'deposit__agriculture', 
			'deposit__canals', 
			'deposit__canal_data', 
			'deposit__dam_data', 
			'deposit__facilities', 
			'deposit__hydrological_data', 
			'deposit__pickup_weir', 
			'deposit__raingauge', 
			'deposit__reservoir_data', 
			'deposit__general_data', 
			'deposit__spillchannel', 
			'deposit__projects'
		);
		$this->db->where('PROJECT_ID', $this->PROJECT_ID);
		$this->db->delete($projectTables);
	...
	private function getValidationDataForLock(){
		...
		$records = $this->db->get_where('deposit__projects', $arrWhich);
		...
		$arrTables = array(
			'deposit__projects_block_served', 'deposit__projects_district_served', 
			'deposit__projects_assembly_const_served'
	...
	public function delproj(){
		$arrTables = array(
			...
			'dep_pmon__m_status_date','dep_pmon__m_setup_status','dep_pmon__m_project_setup','dep_pmon__t_locks','dep_pmon__t_lock_logs','deposit__ip_blockwise',
			'deposit__projects_office','deposit__projects_assembly_const_served','deposit__projects_block_served','deposit__projects_district_served','deposit__projects_villages_served',
			'deposit__agriculture','deposit__canals','deposit__canal_data','deposit__dam_data','deposit__facilities','deposit__hydrological_data',
			'deposit__pickup_weir','deposit__raingauge','deposit__reservoir_data','deposit__spillchannel','deposit__general_data',
			'deposit__t_utility_entry','deposit__t_cca_utility_entry','deposit__actual_irrigation','deposit__projects'
		);
		$this->db->select('PROJECT_ID');
